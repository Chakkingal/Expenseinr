...
async function loadAllData(forceReload = false) {
  ...
  try {
    ...
    // USE CACHE
    if (!forceReload && datasetCache[selected]) {

      const cached = datasetCache[selected];

      expenseData = cached.expense;
      receiptData = cached.receipt;
      contraData = cached.contra;
      openingBalanceData = cached.ob;

    } else {

      const expense = removeEmptyRows(await parseCSV(currentDataset.expenseCSV));
      const receipt = removeEmptyRows(await parseCSV(currentDataset.receiptsCSV));
      const contra = removeEmptyRows(await parseCSV(currentDataset.contraCSV));
      const ob = removeEmptyRows(await parseCSV(currentDataset.obCSV));

      expenseData = expense;
      receiptData = receipt;
      contraData = contra;
      openingBalanceData = ob;

      datasetCache[selected] = {
        expense,
        receipt,
        contra,
        ob
...
function updateSummary() {
  ...
  const exp = applyFilters(expenseData);
  const rec = applyFilters(receiptData);
...
// ==========================================================
// RECEIPT TABLE WITH PASSWORD PROTECTION
// ==========================================================
...
function updateReceiptTable() {
  ...
  let rec = applyFilters(receiptData);

  const start = (receiptPage - 1) * pageSize;
  const pageData = rec.slice(start, start + pageSize);
...
document.getElementById("accountSelector").addEventListener("change", loadAllData);

document.getElementById("refreshBtn").addEventListener("click", () => {

  const selected = document.getElementById("accountSelector").value;

  datasetCache[selected] = null;
